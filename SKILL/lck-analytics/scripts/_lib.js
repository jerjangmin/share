...
async function loadLckResults() {
  ...
  if (process.env.GLOBAL_NPM_ROOT) {
    candidates.push(path.join(process.env.GLOBAL_NPM_ROOT, "lck-results", "src", "index.js"));
  }

  try {
    const globalRoot = await detectGlobalNpmRoot();
    candidates.push(path.join(globalRoot, "lck-results", "src", "index.js"));
  } catch {
    // ignore detection failure and continue to local fallback
  }

  candidates.push(path.resolve(__dirname, "..", "..", "..", "packages", "lck-results", "src", "index.js"));

  const entryPath = candidates.find((candidate) => fs.existsSync(candidate));
  if (!entryPath) {
    throw new Error("Could not find lck-results package. Install it globally with `npm install -g lck-results` or run from the k-skill repo.");
  }
